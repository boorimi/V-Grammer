	...
	<main>
		<div style="display: flex">
			...
			<c:forEach items="${streamIds }" var="s">
				<div>
					<a target="_blank"
						href="https://www.youtube.com/watch?v=${s.address }"> <img
						class="live-icon" src="haco_img/icon/${s.icon }" />
					</a>
				</div>
			</c:forEach>
			<!--  아이콘 배열 끝 -->
			...
			<c:forEach items="${streamIds }" var="s">
				<div class="live-content">
					<iframe width="960" height="540"
						src="https://www.youtube.com/embed/${s.address }" frameborder="0"
				...
				<div class="member-address-container">
					...
					<div class="member-address-box">
						<c:forEach items="${recommendVtuber }" var="rv">
							<div id="${rv.category }">
								<a target="_blank" href="${rv.address }">${rv.category }</a>
							</div>
						</c:forEach>
					</div>
				</div>
